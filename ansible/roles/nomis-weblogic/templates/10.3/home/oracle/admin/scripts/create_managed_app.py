...
jmsFServerDestLocJNDIName=configProps.get("jms.fserver.destination.local.jndi.name") 
jmsFServerDestRemJNDIName=configProps.get("jms.fserver.destination.remote.jndi.name")
jmsFServerFactoryName=configProps.get("jms.fserver.factory.name") 
jmsFServerFactoryLocJNDIName=configProps.get("jms.fserver.factory.local.jndi.name")
jmsFServerFactoryRemJNDIName=configProps.get("jms.fserver.factory.remote.jndi.name")
...
if dsName:
    ...
    for dsName, dsJNDIName in datasources:
        ...
        set('Targets',jarray.array([ObjectName('com.bea:Name='+dsTargetName+',Type=Cluster')], ObjectName))
        cd('/JDBCSystemResources/' + dsName + '/JDBCResource/' + dsName + '/JDBCConnectionPoolParams/' + dsName)
        set('MaxCapacity','300')
        save()
        activate()
...
if jmsModuleName:
    ...
    set('Targets',jarray.array([ObjectName('com.bea:Name='+jmsTarget+',Type=Cluster')], ObjectName))
    save()
    # Create Foreign Server
    cd('/JMSSystemResources/'+jmsModuleName+'/JMSResource/'+jmsModuleName)
    cmo.createForeignServer(jmsFServerName)
    cd('/JMSSystemResources/'+jmsModuleName+'/JMSResource/'+jmsModuleName+'/ForeignServers/'+jmsFServerName)
    cmo.setDefaultTargetingEnabled(true)
    cmo.setInitialContextFactory(jmsFServerContext)
    cmo.createJNDIProperty('datasource')
    cd('/JMSSystemResources/'+jmsModuleName+'/JMSResource/'+jmsModuleName+'/ForeignServers/'+jmsFServerName+'/JNDIProperties/'+'datasource')
    ...
    cd('/JMSSystemResources/'+jmsModuleName+'/JMSResource/'+jmsModuleName+'/ForeignServers/'+jmsFServerName+'/ForeignConnectionFactories/'+jmsFServerFactoryName)
