...
WALLET=/u01/app/oracle/wallets/ssl_wallet
...
get_epoch user
EPOCH_USER_CERT=${EPOCH_DATE}
get_epoch trusted
EPOCH_TRUSTED_CERT=${EPOCH_DATE}
NO_OF_DAYS_BEFORE_USER_EXPIRY=$(( (${EPOCH_USER_CERT} - $(date +%s)) / (60*60*24) ))
NO_OF_DAYS_BEFORE_TRUSTED_EXPIRY=$(( (${EPOCH_TRUSTED_CERT} - $(date +%s)) / (60*60*24) ))

echo "${NO_OF_DAYS_BEFORE_USER_EXPIRY}|${NO_OF_DAYS_BEFORE_TRUSTED_EXPIRY}"