<%@ page session="false" language="java" pageEncoding="UTF-8" contentType="text/html; charset=UTF-8" %>
<!DOCTYPE html>
<html>
<head>
    <meta http-equiv="X-UA-Compatible" content="IE=edge,requiresActiveX=true" charset="UTF-8" />
	<title>Prison-NOMIS Reporting</title>
	<link rel="shortcut icon" type="image/x-icon" href="InfoView.ico" />
    <link rel="stylesheet" href="browsercheck.css">
    <script type="text/javascript" src="browsercheck.js?v=2"></script>
    <script type="text/javascript">
        window.onload = init;
    </script>
</head>
<body>
    <div class="BILP-launchpadbackground">
        <div class="divCenter" id="checking" style="display:inline;">
            Checking if your browser is supported by Prison-NOMIS Reporting...
        </div>
        <div class="divCenter" id="supported" style="display:none;">
            Browser check complete, redirecting you to Prison-NOMIS Reporting...
        </div>
    </div>
</body>
</html>