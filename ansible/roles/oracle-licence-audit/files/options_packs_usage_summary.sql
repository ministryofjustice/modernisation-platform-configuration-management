---------------------------------------------------------------------------------
---------------------------------------------------------------------------------
------- Name        :  options_packs_usage_summary.sql
-------      Based on options_packs_usage_statistics.sql from MOS DOC ID 1317265.1
-------      Just prints the main summary of the options and packs usage
----------------------------------------------------------------------------------
----------------------------------------------------------------------------------
...
define VERSION=to_char(NULL)
col VERSION_ new_val VERSION noprint
select i.VERSION as VERSION_ from V$INSTANCE i, V$DATABASE d, V$VERSION v where v.BANNER like 'Oracle%' or v.BANNER like 'Personal Oracle%';
...
SELECT 'Database Vault'                                      , 'Privilege Capture'                                       , '^1[289]\.|^2[0-9]\.'                          , 'INVALID'       from dual union all -- no longer part of Database Vault, see MOS Doc ID 2588251.1
...
select
  ...
  from PFUS
  where USAGE in ('4.PAST_USAGE', '5.PAST_OR_CURRENT_USAGE', '6.CURRENT_USAGE')   -- ignore '1.NO_PAST_USAGE', '2.NO_CURRENT_USAGE', '3.SUPPRESSED_DUE_TO_BUG'
