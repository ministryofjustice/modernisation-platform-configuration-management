CREATE OR REPLACE PACKAGE BODY AUDITDATA.AUDITDATA_DML_PKG AS
...
--*/
  ...
  */
	...
	v_handler 	varchar2(80) := g_data_owner || '.AUDITDATA_DML_PKG.' || g_handler_name;
	v_dblink 	varchar2(128) := 'T1CNOM.REGRESS.RDBMS.DEV.US.ORACLE.COM';
	v_scn 		number := null;
	v_table 	varchar2(30) := null;
	v_date 		date := null;
  BEGIN
	--get source database link name
--	v_plsql := 'select db_link from dba_db_links where owner = :user and username = :ggint';
...
--	execute immediate v_plsql into v_dblink, v_scn, v_date using user, 'GGINT';
	...
	if v_tab_list.count > 0 then
		...
		loop
			...
			-- If we set apply_name to NULL then the handler is used for ALL apply & inbound servers
			v_plsql := 
				'begin
...
/*
  ...
  BEGIN
    ...
    BEGIN
        SYS.DBMS_CAPTURE_ADM.STOP_CAPTURE(g_capture_name, p_force);
    EXCEPTION
    ...
    audit_log_error(0, 'Capture process ' || g_capture_name || ' stopped' || v_retry_txt || '.',
		    v_msg_number, null, null, null, null, null, 'stop_capture');
  EXCEPTION
    WHEN OTHERS THEN
	audit_log_error(SQLCODE, SQLERRM, v_msg_number, null, null, null, null, null, 'stop_capture');
        RAISE;
  END;
  ...
  BEGIN
    ...
    SYS.DBMS_CAPTURE_ADM.START_CAPTURE(g_capture_name);
    -- log message
    audit_log_error(0, 'Capture process ' || g_capture_name || ' started.',
 		    null, null, null, null, null, null, 'start_capture');
  EXCEPTION
    WHEN OTHERS THEN
	audit_log_error(SQLCODE, SQLERRM, null, null, null, null, null, null, 'start_capture');
        RAISE;
  END;
*/
...
/*
-- TO DO: stop the Replicat through a script instead
    BEGIN
        SYS.DBMS_APPLY_ADM.STOP_APPLY(g_apply_name, p_force);
    EXCEPTION
        WHEN timeout_stopping_process THEN
...
*/
    ...
    audit_log_error(0, 'Apply process ' || g_apply_name || ' stopped' || v_retry_txt || '.',
 		    v_msg_number, null, null, null, null, v_msg_date, 'stop_apply');
  EXCEPTION
    WHEN OTHERS THEN
		audit_log_error(SQLCODE, SQLERRM, v_msg_number, null, null, 
			null, null, v_msg_date, 'stop_apply');
        RAISE;
  END;
  ...
  BEGIN
    ...
    END IF;
--    SYS.DBMS_APPLY_ADM.START_APPLY(g_apply_name);
-- TO DO: start the Replicat through a script instead but call this first to set up the DML handlers
    
    -- log message
    audit_log_error(0, 'Apply process ' || g_apply_name || ' started.',
 		    null, null, null, null, null, null, 'start_apply');
...
--    WHEN OTHERS THEN
--	audit_log_error(SQLCODE, SQLERRM, null, null, null, null, null, null, 'start_apply');
--        RAISE;
  END;
