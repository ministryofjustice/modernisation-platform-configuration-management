CREATE OR REPLACE PACKAGE BODY MIS_BATCH_PKG AS
/*
...
--    stop_apply(v_staging_owner);
    ...
    IF (p_restart = TRUE) AND (v_apply_error_count = 0) AND
       (v_apply_proc_error_count = 0) AND (v_capture_error_count = 0) THEN
        ...
        -- restart apply
-- can't start ogg replicat from the package so need to do that after returning from this procedure
--        start_apply(v_staging_owner);
        update_batch_record(v_new_batch_number, null, null, 'Apply underway');
        -- restart capture in case stopped due to SCN limit
-- can't start ogg capture from the package so need to do that after returning from this procedure
/*
        BEGIN
    	    start_capture(p_staging_owner);
        EXCEPTION
