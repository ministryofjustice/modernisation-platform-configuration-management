...
def ServerState(server):
  if server != 'wls':
      nmConnect(userConfigFile=nmConfigFile,userKeyFile=nmKeyFile,port=nmPort,host=nmMachines,domainName=domainName,domainDir=domainHome,nmType='Plain')
...
def nmStartNM(machine):
   try:
      ...
      startNodeManager(verbose='false', NodeManagerHome=nmHome, ListenPort=nmPort, ListenAddress=nmMachines)
