...
def run_batch_file(target, port, username, password, batch_file):
    try:
        ...
        if r.status_code == 0:
            ...
            print("Unable to execute the batch file. Status code: {}".format(r.status_code))
    except Exception as e:
        print("An error occurred while trying to execute the batch file:", str(e))

def main():
    parser = argparse.ArgumentParser(description='Run a batch file on target machine via WinRM')
    ...
    parser.add_argument('-b', '--batch', help='Path to the batch file on the target machine', required=True)

    args = parser.parse_args()

    run_batch_file(args.target, args.port, args.username, args.password, args.batch)
