...
$ShortcutPath = Join-Path -Path $ShortcutDir -ChildPath "Designer (common config run as Admin).lnk"
...
$ShortcutPath = Join-Path -Path $ShortcutDir -ChildPath "Designer (individual user config).lnk"
if (-not (Test-Path $ShortcutPath)) {
  Write-Output "Creating Desktop Shortcut: $ShortcutPath"
  $WScriptShell = New-Object -ComObject WScript.Shell
  $Shortcut = $WScriptShell.CreateShortcut($ShortcutPath)
...
$ShortcutPath = Join-Path -Path $ShortcutDir -ChildPath "Information Design Tool (common config run as Admin).lnk"
if (-not (Test-Path $ShortcutPath)) {
  Write-Output "Creating Desktop Shortcut: $ShortcutPath"
  $WScriptShell = New-Object -ComObject WScript.Shell
  $Shortcut = $WScriptShell.CreateShortcut($ShortcutPath)
...
$ShortcutPath = Join-Path -Path $ShortcutDir -ChildPath "Information Design Tool (individual user config).lnk"
if (-not (Test-Path $ShortcutPath)) {
  Write-Output "Creating Desktop Shortcut: $ShortcutPath"
  $WScriptShell = New-Object -ComObject WScript.Shell
  $Shortcut = $WScriptShell.CreateShortcut($ShortcutPath)
